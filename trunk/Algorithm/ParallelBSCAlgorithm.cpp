...
ParallelBSCAlgorithm::prepareDataForThread (int parent, int child, int fork) {
    ...
    if (fork > 0)
      T[child]->prevColor      = T[parent]->A[fork-1].colors;  
    else
      T[child]->prevColor      = T[parent]->prevColor;  
...
ParallelBSCAlgorithm::colourGraph(void* c){
    ...
    //while(start >= 0) {
    while(start >= forkPoint) {

      back = BACK_GOOD;
      beforeRollBack = start;
      //cout << "T" << t << " restarting at " << start << endl;

   //   for (int j = 0; j < size; j++) {
   //   cout << "T" << t << ": H" << j << ": P" << heap[j].parent
   //                                  << ": L" << heap[j].left
   //                                  << ": R" << heap[j].right << endl;
   //   }
      ...
      for (int i = start; i < size; i++) {
         ...
         if (i > start) {
           ...
           //if (colours[root] > 0) cout << "T" << t << ": V" << root << " is alrdy" << endl;
           // print out the heap in the last 2 iterations
          // if (i >= size - 2) {
          //   for (int j = 0; j < size; j++) {
          //   cout << "T" << t << ": H" << j << ": P" << heap[j].parent
          //                                  << ": L" << heap[j].left
      ...
      switch (back) {
       ...
       case BACK_GOOD:
        ...
        for (int i = 0; i < g_->getSize(); i++) {
           //cout << "T" << t << ": V" << i << ": Colour:" << colours[i] << endl;
           Fopt[i] = colours[i]; 
        }
    ...
    while (!T[t]->spawn.empty()) {
       ...
       //cout << "Taking " << childThread << " results" << endl;
          optColorNumber = T[childThread]->holdingFopt;
